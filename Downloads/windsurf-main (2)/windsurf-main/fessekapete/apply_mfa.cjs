...
const handleSubmitMatch = 'const handleSubmit = async e => {';
...
content = content.replace(handleSubmitMatch, handleMfaSubmit);
...
// Replace `{!isRecoveryMode && (` with `{!mfaResolver && !isRecoveryMode && (` and also inject the `mfaResolver && (...)` before it.
const formStartMatch = '{!isRecoveryMode && (';
content = content.replace(formStartMatch, mfaFormUI);

fs.writeFileSync(filePath, content, 'utf8');
console.log("MFA UI successfully injected into Login.jsx");
